...
class Fakename < Linkbot::Plugin
  ...
  def self.help
    "!fakename - generate a fake name"
  end

  Linkbot::Plugin.register('fakename', self,
    {
      :message => {:regex => /!fakename/, :handler => :on_message, :help => :help},
      :"direct-message" => {:regex => /!fakename/, :handler => :on_message, :help => :help}
    }
  )
end
